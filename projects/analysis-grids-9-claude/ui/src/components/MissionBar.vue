...
<template>
  <div
    ...
    class="mission-bar"
  >
    <span class="mission-label" title="Current mission objective and status">Mission</span>
    <span class="mission-target" title="Total basalt amount to collect for mission completion">collect {{ target }} basalt</span>
    <span
      class="mission-progress-wrap"
      title="Progress: {{ collected }} out of {{ target }} basalt collected ({{ progressPct }}%)"
    >
    ...
    <span
      ...
      :title="`Cargo in transit to station: ${mission.in_transit_quantity} units`"
    >{{ mission.in_transit_quantity }} in transit</span>
    <span
      class="mission-status"
      :class="mission.status"
      :title="`Mission status: ${mission.status === 'running' ? 'Active - collect basalt' : mission.status === 'success' ? 'Completed' : 'Ended'}`"
    >{{ mission.status }}</span>
    <button
      v-if="mission.status === 'running'"
      class="abort-btn"
      @click="emit('abort')"
      title="Emergency abort: terminate mission immediately"
    >
      ABORT
    </button>
  </div>
