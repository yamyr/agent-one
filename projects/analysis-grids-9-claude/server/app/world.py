...
def _storm_level_to_visibility(storm_level):
    """Convert storm level to visibility factor."""
    visibility_map = {0: 1.0, 1: 0.9, 2: 0.7, 3: 0.4}
    return visibility_map.get(storm_level, 1.0)


def _storm_level_to_temperature_effect(storm_level):
    """Convert storm level to temperature effect (in Celsius)."""
    # Storms make it colder on Mars
    temp_effect_map = {0: 0, 1: -5, 2: -15, 3: -30}
    return temp_effect_map.get(storm_level, 0)


def trigger_storm_event():
    """Trigger a random storm event and update the world state."""
    import random

    # Only trigger if no storm is currently active
    if WORLD["environment"]["storm_active"]:
        return False

    # Generate random storm level (1-3, never 0)
    storm_level = random.randint(1, 3)
    duration = random.randint(10, 30)  # Storm lasts 10-30 ticks

    # Update environment
    WORLD["environment"]["storm_level"] = storm_level
    WORLD["environment"]["storm_active"] = True
    WORLD["environment"]["storm_duration"] = duration
    WORLD["environment"]["visibility"] = _storm_level_to_visibility(storm_level)

    # Base temperature is -35C, storms make it colder
    base_temp = -35
    temp_effect = _storm_level_to_temperature_effect(storm_level)
    WORLD["environment"]["temperature"] = base_temp + temp_effect

    # Schedule next storm (after current storm ends + some time)
    WORLD["environment"]["next_storm_tick"] = WORLD["tick"] + duration + random.randint(20, 50)

    logger.info(f"Storm event triggered: level {storm_level}, duration {duration} ticks")

    # Broadcast storm event to all agents
    from .protocol import make_message
    from .broadcast import broadcaster
    import asyncio

    async def broadcast_storm():
        msg = make_message(
            source="world",
            type="event",
            name="storm_event",
            payload={
                ...
                "duration": duration,
                "visibility": WORLD["environment"]["visibility"],
    ...
    try:
        asyncio.create_task(broadcast_storm())
    except RuntimeError:
...
def update_storm_state():
    """Update storm state each tick - decrement duration, end storm when done."""
    env = WORLD["environment"]

    if env["storm_active"]:
        env["storm_duration"] -= 1
        if env["storm_duration"] <= 0:
            # End storm
            env["storm_level"] = 0
            env["storm_active"] = False
            env["visibility"] = 1.0
            env["temperature"] = -35  # Return to base temperature
            logger.info("Storm event ended")
            # Schedule next storm
            env["next_storm_tick"] = WORLD["tick"] + random.randint(30, 80)
            ...
            async def broadcast_storm_end():
                msg = make_message(
                    source="world",
                    type="event",
                    name="storm_ended",
                    payload={"storm_level": 0, "visibility": 1.0, "temperature": -35},
                )
                await broadcaster.send(msg.to_dict())

            # Run the broadcast in the background
            try:
                asyncio.create_task(broadcast_storm_end())
            except RuntimeError:
                # No event loop running, skip broadcast
                pass
...
def _build_initial_world():
    ...
    world = {
        ...
        "environment": {
            "storm_level": 0,  # 0 = clear, 1 = mild, 2 = moderate, 3 = severe
            "visibility": 1.0,  # 1.0 = full visibility, reduced during storms
            "temperature": -35,  # Celsius
            "storm_active": False,  # Whether a storm is currently active
            "storm_duration": 0,  # Remaining duration of current storm
            "next_storm_tick": 30,  # When the next storm will occur (tick-based)
        },
        "agents": {
            "station": {
                "position": [0, 0],
...
def next_tick():
    ...
    # Update storm state each tick
    update_storm_state()

    # Check if it's time to trigger a new storm
    if (
        not WORLD["environment"]["storm_active"]
        and WORLD["tick"] >= WORLD["environment"]["next_storm_tick"]
    ):
        trigger_storm_event()
